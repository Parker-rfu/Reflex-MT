...
class DataParallelCOMETRefFree(BaseCOMETRefFreeModel):
    ...
    def extract_draft_translation(self, solution_str: str):
        """【新增】提取 draft translation
        
        Args:
            solution_str: Raw response string from the language model
            
        Returns:
            Tuple containing (draft_answer, processed_string)
        """
        # Split response to isolate assistant output
    ...
    def compute_comet_free_rm_draft(self, data: DataProto) -> torch.Tensor:
        ...
        for i in range(len(data)):
            ...
            # 【关键】提取 draft answer 而非 final answer
            draft_answer, processed_str = self.extract_draft_translation(sequences_str)

            src_text = data_item.non_tensor_batch['src_text']
            
            if draft_answer:
                new_item = {"src": src_text, "mt": draft_answer}
            else:
                # 如果没有找到 draft，使用空字符串或降级处理
                new_item = {"src": src_text, "mt": "None"}
                print(f"[Warning] No draft found for sample {i}")
