...
class COMETWorker(Worker):
    ...
    def init_model(self):
        ...
        tokenizer_path = copy_local_path_from_hdfs(self.config.tokenizer_path)
        self.tokenizer = hf_tokenizer(tokenizer_path)

        # 检查ckpt_path是否以'.ckpt'结尾
        if not self.config.ckpt_path.endswith('.ckpt'):
            self.comet_module = load_from_checkpoint(self.config.ckpt_path + '/model.ckpt', reload_hparams=True)
        else:
            self.comet_module = load_from_checkpoint(self.config.ckpt_path, reload_hparams=True)
    ...
    def compute_comet_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(tensors={'comet_rm': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
        torch.cuda.empty_cache()
        return output
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_comet_rm_draft(self, data: DataProto):
        """【新增】计算 draft answer 的 COMET 评分"""
        data = data.to('cuda')
        micro_batch_size = self.config.forward_micro_batch_size
        data.meta_info['micro_batch_size'] = micro_batch_size
        data.meta_info['use_dynamic_bsz'] = self.config.use_dynamic_bsz
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_comet_rm_draft(data=data)
            output = DataProto.from_dict(tensors={'comet_rm_draft': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
        torch.cuda.empty_cache()
        return output
...
class COMETFreeWorker(Worker):
    ...
    def init_model(self):
        # This is used to import external_lib into the huggingface systems
        import_external_libs(self.config.model.get('external_lib', None))
        from verl.workers.cometfree import DataParallelCOMETRefFree
        from comet import load_from_checkpoint
        ...
        if not self.config.ckpt_path.endswith('.ckpt'):
            ...
            self.comet_module = load_from_checkpoint(self.config.ckpt_path, reload_hparams=True)
        self.comet = DataParallelCOMETRefFree(config=self.config, comet_module=self.comet_module, tokenizer = self.tokenizer)

        torch.cuda.empty_cache()

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_comet_free_rm(self, data: DataProto):
        data = data.to('cuda')
        micro_batch_size = self.config.forward_micro_batch_size
        data.meta_info['micro_batch_size'] = micro_batch_size
        data.meta_info['use_dynamic_bsz'] = self.config.use_dynamic_bsz
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_comet_free_rm(data=data)
            output = DataProto.from_dict(tensors={'comet_free_rm': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
        torch.cuda.empty_cache()
        return output
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_comet_free_rm_draft(self, data: DataProto):
        """【新增】计算 draft answer 的 COMET-Free 评分"""
        data = data.to('cuda')
        micro_batch_size = self.config.forward_micro_batch_size
        data.meta_info['micro_batch_size'] = micro_batch_size
        data.meta_info['use_dynamic_bsz'] = self.config.use_dynamic_bsz
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_comet_free_rm_draft(data=data)
            output = DataProto.from_dict(tensors={'comet_free_rm_draft': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
        torch.cuda.empty_cache()
        return output
    ...
    @register(dispatch_mode=Dispatch.ALL_TO_ALL, execute_mode=Execute.RANK_ZERO)
    def compute_valid_comet_free(self, data: DataProto):
        data = data.to('cuda')
        micro_batch_size = self.config.forward_micro_batch_size
        data.meta_info['micro_batch_size'] = micro_batch_size
        data.meta_info['use_dynamic_bsz'] = self.config.use_dynamic_bsz
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_valid_comet_free(data=data)
            output = DataProto.from_dict(tensors={'valid_comet_free_metric': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
        torch.cuda.empty_cache()
        return output
