...
def extract_solution(solution_str: str, template_type: str = 'base') -> Tuple[str, str]:
    """Extracts the final answer from the model's response string.
    
    Args:
        solution_str: Raw response string from the language model
        template_type: Template type to determine extraction method
    ...
    # Extract final answer using XML-style tags based on template type
    if template_type == 'reflex':
        # For reflex format, try to extract final translation first, then draft as fallback
        final_pattern = r'<final>(.*?)</final>'
        final_matches = list(re.finditer(final_pattern, processed_str, re.DOTALL))
        
        if final_matches:
            final_answer = final_matches[-1].group(1).strip()
            return final_answer, processed_str
        else:
            # Fallback to draft if final not found
            draft_pattern = r'<draft>(.*?)</draft>'
            draft_matches = list(re.finditer(draft_pattern, processed_str, re.DOTALL))
            
            if draft_matches:
                draft_answer = draft_matches[-1].group(1).strip()
                print("[Warning] Using draft translation as final was not found")
                return draft_answer, processed_str
            else:
                print("[Error] No valid draft or final tags found in reflex format")
                return None, processed_str
    else:
        # Original logic for base/rl formats
...
def process_data(input_data, llm, sampling_params, tokenizer, template_type):
    ...
    for i, output in enumerate(outputs):
        ...
        # 提取最终翻译结果
        extracted_answer, processed_str = extract_solution(prompts[i] + generated_text, template_type)
        if i == 0:
...
def main():
    ...
    parser.add_argument("--template-type", type=str, default="base", 
                        choices=["base", "chat", "rl", "reflex"], 
                        help="提示模板类型: base, chat, rl, 或 reflex")
